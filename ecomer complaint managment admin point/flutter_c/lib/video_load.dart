...
class VideoLoadPage extends StatefulWidget {
  @override
  _VideoLoadPageState createState() => _VideoLoadPageState();
}

class _VideoLoadPageState extends State<VideoLoadPage> {
 ...
 // String _selectedCategory = 'Flower farming';
  ...
  Future<void> _fetchVideos() async {
    QuerySnapshot snapshot = await _firestore.collection('videosload').get();
    setState(() {
  ...
  Future<void> _submitVideoUrl() async {
    ...
    if (videoId != null) {
      ...
      try {
        await _firestore.collection('videosload').add({
  ...
  Future<void> _deleteVideo(String docId) async {
    try {
      await _firestore.collection('videosload').doc(docId).delete();
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
